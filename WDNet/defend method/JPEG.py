...
def jpeg_compress(xs, x_min, x_max, quality=10):
...
for index in range(100):
    ...
    wm_np = wm_np.transpose((0, 2, 3, 1))
    wm_tf = tf.constant(wm_np)
    wm_tf = jpeg_compress(wm_tf, 0, 1)
    wm_np = np.array(wm_tf).transpose((0, 3, 1, 2))
    wm = torch.tensor(wm_np).cuda()
    r1 = wm
    ...
    '''compress'''
    wm_r_np = wm_r.detach().cpu().numpy()
    wm_r_np = wm_r_np.transpose((0, 2, 3, 1))
    wm_r_tf = tf.constant(wm_r_np)
    wm_r_tf = jpeg_compress(wm_r_tf, 0, 1)
    wm_r_np = np.array(wm_r_tf).transpose((0, 3, 1, 2))
    wm_r = torch.tensor(wm_r_np).cuda()
