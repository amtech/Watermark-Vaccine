...
total_delta1_wdnet = torch.load('./delta1_wdnet.pt')
total_delta2_wdnet = torch.load('./delta2_wdnet.pt')
total_delta1_split = torch.load('./delta1_split.pt')
total_delta2_split = torch.load('./delta2_split.pt')
total_delta1_bvmr = torch.load('./delta1_bvmr.pt')/255.0
total_delta2_bvmr = torch.load('./delta2_bvmr.pt')/255.0

total_delta1 =  total_delta1_wdnet
total_delta2 =  total_delta2_wdnet

total_delta1 = clamp(total_delta1, -epsilon, epsilon)
total_delta2 = clamp(total_delta2, -epsilon, epsilon)
...
for index in range(200):
    ...
    #adv1
    delta1 = total_delta1[index].unsqueeze(0)
    ...
    delta2 = total_delta2[index].unsqueeze(0)
    ...
    print('clean image:psnr_h: %.4f, ssim_h: %.4f, rmse_all_h: %.4f,rmse_in_h: %.4f' % (ans_psnr_h / t, ans_ssim_h / t, rmse_all_h / t,rmse_in_h / t,))
    print('clean image:psnr_w: %.4f, ssim_w: %.4f, rmse_all_w: %.4f,rmse_in_w: %.4f' % (ans_psnr_w / t, ans_ssim_w / t,  rmse_all_w / t,rmse_in_w / t,))
    ...
    print('random image:psnr_h: %.4f, ssim_h: %.4f, rmse_all_h: %.4f, rmse_in_w: %.4f' % (r_ans_psnr_h / t, r_ans_ssim_h / t, r_rmse_all_h / t,r_rmse_in_h / t))
    print('random image:psnr_w: %.4f, ssim_w: %.4f, rmse_all_h: %.4f, rmse_in_w: %.4f' % (r_ans_psnr_w / t, r_ans_ssim_w / t, r_rmse_all_w / t,r_rmse_in_w / t))
    ...
    print('Max-loss image:psnr_h: %.4f, ssim_h: %.4f, rmse_all: %.4f, rmse_in: %.4f' % (a1_ans_psnr / t, a1_ans_ssim / t, a1_rmse_all / t,a1_rmse_in / t))
